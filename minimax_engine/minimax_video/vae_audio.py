...
#
# Ported from huggingface/diffusers#14355 @ e1b518d (autoencoder_kl_minimax_h3_audio.py) for H1111.
# Rewritten against the pinned diffusers==0.33.1: attention machinery comes from the local
# `attention` module and the missing `AutoencoderMixin` is replaced by a local shim.
...
def get_parameter_dtype(module: nn.Module) -> torch.dtype:
    """First floating-point parameter/buffer dtype of a plain submodule.

    The 0.33.1 `diffusers.models.modeling_utils.get_parameter_dtype` assumes it is called on a
    `ModelMixin` (it reads `parameter._keep_in_fp32_modules`); this file calls it on `self.encoder`
    / `self.decoder`, which are plain `nn.Module`s, so a local equivalent is used instead.
    """
    last_dtype = None
    ...
    if last_dtype is None:
        raise ValueError("Module has no parameters or buffers to read a dtype from.")
    return last_dtype
