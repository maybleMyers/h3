...
(DNN for encoding / decoding videos to Hunyuan Video's latent space)
 From: https://github.com/madebyollin/taehv @ 62f7591
 MIT License
Supported checkpoints (detected from filename): taehv.pth (Hunyuan), taehv1_5[_super].pth,
taew2_1.pth / taew2_2[_super].pth (Wan), taecvx.pth (CogVideoX), taeltx*.pth (LTX),
taeos1_3.pth, taeh3.pth (MiniMax-H3).
Local divergence from upstream: checkpoint loading also accepts .safetensors files,
and the roundtrip CLI (main) is omitted.
"""
import torch
